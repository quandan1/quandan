...
<body>
	<div id="list" style=" display: block; overflow:auto; width:100%; ">
	    ...
	    <div style="padding: 2px 8px; float:right; border: solid; border-width: 1px; border-color: #eaeafa; ">
			...
			<select name="type" id="type" style="margin-right: 5px;width: 170px;text-align: center" title="指标分析">
				...
				<option value="KZDM&HXXYL&化学需氧量统计&断面化学需氧量比较&化学需氧量（吨）">化学需氧量（吨）</option>
                <option value="KZDM&ADPFL&氨氮排放量统计&断面氨氮排放量比较&氨氮排放量（吨）">氨氮排放量（吨）</option>
			</select>
		...
		<table width="100%" border="0" cellpadding="7" cellspacing="0"
			...
			<tr>
				<th align="center" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">控制断面名称</th>
				<th align="center" value="loc_city" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">所属市</th>
				<th align="center" value="loc_county" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">所属县</th>
	...
	<div class="mclear">
		<div align="center" id="searchPage">
						<font size="2">共 ${page.totalPageCountZcfg} 页</font> <font size="2">第
							${page.pageNow} 页</font> <a href="rest/qmp/sum?pageNow=1"
							class='search_A'>首页</a>
						<c:choose>
							<c:when test="${page.pageNow - 1 > 0}">
								<a href="rest/qmp/sum?pageNow=${page.pageNow - 1}"
									class='search_A'>上一页</a>
							</c:when>
							<c:when test="${page.pageNow - 1 <= 0}">
								<!-- <a href="rest/qmp/sum?pageNow=1" class='search_A'>上一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPageCountZcfg==0}">
								<a href="rest/qmp/sum?pageNow=${page.pageNow}"
							...
							<c:when test="${page.pageNow + 1 > page.totalPageCountZcfg}">
								<!-- <a href="rest/qmp/sum?pageNow=${page.totalPageCount}"
									class='search_A'>下一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							...
							<c:otherwise>
								<a href="rest/qmp/sum?pageNow=${page.totalPageCountZcfg}"
									class='search_A'>尾页</a>
							</c:otherwise>
						</c:choose>
	...
	<script type="text/javascript" src="app/js/qmp/kzdmTotal.js"></script>
</body>
</html>