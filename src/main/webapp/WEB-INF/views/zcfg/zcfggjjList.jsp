	...
	<div id="list" style="margin-top:20px;" class="abc">
	 <table width="100%" border="0" cellpadding="7" cellspacing="0"
		...
		<c:when test="${fn:contains(zcfg.type, '-FILE')}">
		   <tr>
		    <td align="center" style="white-space: nowrap;"><img src="app/img/fold.png" height="20" width="20"/></td>
		    <td align="center"><a href="rest/page/zcfgGjjA?${zcfg.title}" id="${zcfg.title}" class='fold'>${zcfg.name}</a></td>
		...
		<c:otherwise>
		   <tr>
                ...
                <td align="center" style="white-space: nowrap;">
				<a href="rest/Zcfg/delete?id=${zcfg.id}" id="deleteButtonzGjj" class="updateButton" style="color: white;background:#FAAD5E;padding:2px 6px" name="${zcfg.name}">删除</a>
           ...
           <!-- 分页功能 start -->  
			   		<div align="center" id="gjjPage">
						<font size="2">共 ${page.totalPageCountZcfg} 页</font> <font size="2">第
							${page.pageNow} 页</font> <a href="rest/Zcfg/selectGJJ?pageNow=1">首页</a>
						<c:choose>
							<c:when test="${page.pageNow - 1 > 0}">
								<a href="rest/Zcfg/selectGJJ?pageNow=${page.pageNow - 1}">上一页</a>
							</c:when>
							<c:when test="${page.pageNow - 1 <= 0}">
								<!-- <a href="rest/Zcfg/selectGJJ?pageNow=1">上一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPageCountZcfg==0}">
								<a href="rest/Zcfg/selectGJJ?pageNow=${page.pageNow}">下一页</a>
							</c:when>
							<c:when test="${page.pageNow + 1 <= page.totalPageCountZcfg}">
								<a href="rest/Zcfg/selectGJJ?pageNow=${page.pageNow + 1}">下一页</a>
							</c:when>
							<c:when test="${page.pageNow + 1 > page.totalPageCountZcfg}">
								<!-- <a href="rest/Zcfg/selectGJJ?pageNow=${page.totalPageCount}">下一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPageCountZcfg==0}">
								<a href="rest/Zcfg/selectGJJ?pageNow=${page.pageNow}">尾页</a>
							</c:when>
							<c:otherwise>
								<a href="rest/Zcfg/selectGJJ?pageNow=${page.totalPageCountZcfg}">尾页</a>
							</c:otherwise>
						</c:choose>
