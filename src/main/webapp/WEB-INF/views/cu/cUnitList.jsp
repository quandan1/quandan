...
<body>
	<div id="list" style="display: block;overflow:auto;width:100%;">
		<table width="100%" border="0" cellpadding="7" cellspacing="0"
			...
			<c:forEach var="controlUnit" items="${clist}">
				<tr id="cux${controlUnit.id}">
					<td align="center" style="white-space: nowrap;">
							<a href="rest/controlunit/id?id=${controlUnit.id}" class="updateButton" style="color: white;background:#FAAD5E;padding:2px 6px">修改</a> 
							<a href="rest/controlunit/delete?id=${controlUnit.id}" class="deleteButton" style="color: white;background:#00C0BF;padding:2px 6px">删除</a>
					</td>
            ...
            <!-- 分页功能 start -->  
			    <div align="center" id="page">  
			        <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第  
			            ${page.pageNow} 页</font> <a href="rest/controlunit/list?pageNow=1">首页</a>  
			        <c:choose>  
			            <c:when test="${page.pageNow - 1 > 0}">  
			                <a href="rest/controlunit/list?pageNow=${page.pageNow - 1}">上一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow - 1 <= 0}">  
			        ...
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">  
			                <a href="rest/controlunit/list?pageNow=${page.pageNow}">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 <= page.totalPageCount}">  
			                <a href="rest/controlunit/list?pageNow=${page.pageNow + 1}">下一页</a>  
			            </c:when>  
			            <c:when test="${page.pageNow + 1 > page.totalPageCount}">  
			        ...
			        <c:choose>  
			            <c:when test="${page.totalPageCount==0}">  
			                <a href="rest/controlunit/list?pageNow=${page.pageNow}">尾页</a>  
			            </c:when>  
			            <c:otherwise>  
			                <a href="rest/controlunit/list?pageNow=${page.totalPageCount}">尾页</a>  
			            </c:otherwise>  
			        </c:choose>  
			    </div>  
			<!-- 分页功能 end -->  
