...
<body>
	<div id="list" style=" display: block; overflow:auto; width:100%; ">
		...
		<div style="padding: 2px 8px; float:right; border: solid; border-width: 1px; border-color: #eaeafa; ">
			...
			<select name="type" id="type" style="margin-right: 5px;width: 170px;text-align: center" title="指标分析">
				...
				<option value="KZDY&HXXYLCSL_D&化学需氧量产生量统计&单元化学需氧量产生量比较&化学需氧量产生量（吨）">化学需氧量产生量（吨）</option>
				<option value="KZDY&HXXYLPFL_D&化学需氧量排放量统计&单元化学需氧量排放量&化学需氧量排放量（吨）">化学需氧量排放量（吨）</option>
				<option value="KZDY&ADCSL_D&氨氮产生量统计&单元氨氮产生量比较&氨氮产生量（吨）">氨氮产生量（吨）</option>
				<option value="KZDY&ADPFL_D&氨氮排放量统计&单元氨氮排放量比较&氨氮排放量（吨）">氨氮排放量（吨）</option>
				<option value="KZDY&SYLCSL_D&石油量产生量统计&单元石油量产生量比较&石油量产生量（吨）">石油量产生量（吨）</option>
				<option value="KZDY&SYLPFL_D&石油排放量统计&单元石油量排放量比较&石油量排放量（吨）">石油量排放量（吨）</option>
				<option value="KZDY&GYFSPFL_D&工业废水排放量统计&单元工业废水排放量比较&工业废水排放量（吨）">工业废水排放量（吨）</option>
		...
		<table width="100%" border="0" cellpadding="7" cellspacing="0"
			...
			<tr>
				<th align="center" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">污染源控制单元名称</th>
	...
	<div class="mclear">
		<div align="center" id="searchPage">
						<font size="2">共 ${page.totalPageCountZcfg} 页</font> <font size="2">第
							${page.pageNow} 页</font> <a href="rest/controlunit/total?pageNow=1"
							class='search_A'>首页</a>
						<c:choose>
							<c:when test="${page.pageNow - 1 > 0}">
								<a href="rest/controlunit/total?pageNow=${page.pageNow - 1}"
									class='search_A'>上一页</a>
							</c:when>
							<c:when test="${page.pageNow - 1 <= 0}">
								<!-- <a href="rest/controlunit/total?pageNow=1" class='search_A'>上一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPageCountZcfg==0}">
								<a href="rest/controlunit/total?pageNow=${page.pageNow}"
							...
							<c:when test="${page.pageNow + 1 > page.totalPageCountZcfg}">
								<!-- <a href="rest/controlunit/total?pageNow=${page.totalPageCount}"
									class='search_A'>下一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							...
							<c:otherwise>
								<a href="rest/controlunit/total?pageNow=${page.totalPageCountZcfg}"
									class='search_A'>尾页</a>
							</c:otherwise>
						</c:choose>
	...
	<script type="text/javascript" src="app/js/cu/kzdySource.js"></script>
</body>
</html>