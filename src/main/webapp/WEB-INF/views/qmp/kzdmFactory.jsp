...
<body>
	<div id="list" style=" display: block; overflow:auto; width:100%; ">
	    ...
	    <div style="padding: 2px 8px; float:right; border: solid; border-width: 1px; border-color: #eaeafa; ">
			...
			<select name="type" id="type" style="margin-right: 5px;width: 170px;text-align: center" title="指标分析">
				...
				<!-- <option value="KZDM&WSSJCLNL_D_R&污水设计处理能力统计&断面污水设计处理能力比较&污水设计处理能力（吨/日）">污水设计处理能力（吨/日）</option>
				<option value="KZDM&WSSJCLL_WD&污水实际处理量统计&断面污水实际处理量&污水实际处理量（万吨）">污水实际处理量（万吨）</option> -->
				<option value="KZDM&HXXYLQCL_D&化学需氧量去除量统计&断面化学需氧量去除量比较&化学需氧量去除量（吨）">化学需氧量去除量（吨）</option>
				<option value="KZDM&HXXYLJKND_HK_S&化学需氧量进口浓度统计&断面化学需氧量进口浓度比较&化学需氧量进口浓度（毫克/升）">化学需氧量进口浓度（毫克/升）</option>
				<option value="KZDM&HXXYLCKND_HK_S&化学需氧量出口浓度统计&断面化学需氧量出口浓度比较&化学需氧量出口浓度（毫克/升）">化学需氧量出口浓度（毫克/升）</option>
                 <option value="KZDM&ADQCL_D&氨氮去除量统计&断面氨氮去除量比较&氨氮去除量（吨）">氨氮去除量（吨）</option>
                <option value="KZDM&ADJKND_HK_S&氨氮进口浓度统计&断面氨氮进口浓度比较&氨氮进口浓度（毫克/升）">氨氮进口浓度（毫克/升）</option>
                <option value="KZDM&ADCKND_HK_S&氨氮出口浓度统计&断面氨氮出口浓度比较&氨氮出口浓度（毫克/升）">氨氮出口浓度（毫克/升）</option>
			</select>
		...
		<table width="100%" border="0" cellpadding="7" cellspacing="0"
			...
			<tr>
				<th align="center" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">污水处理厂控制断面名称</th>
				...
				<th align="center" id="elem1" value="HXXYLQCL_D" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">化学需氧量去除量（吨）</th>
				<th align="center" id="elem2" value="HXXYLJKND_HK_S" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">化学需氧量进口浓度（毫克/升）</th>
				<th align="center" id="elem3" value="HXXYLCKND_HK_S" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">化学需氧量出口浓度（毫克/升）</th>
			    <th align="center" id="elem4" value="ADQCL_D" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">氨氮去除量（吨）</th>
				<th align="center" id="elem5" value="ADJKND_HK_S" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">氨氮进口浓度（毫克/升）</th>
				<th align="center" id="elem6" value="ADCKND_HK_S" style="white-space: nowrap;text-align: center;background:#99A4AF;color:white">氨氮出口浓度（毫克/升）</th>
			</tr>
	...
	<div class="mclear">
		<div align="center" id="searchPage">
						<font size="2">共 ${page.totalPageCountZcfg} 页</font> <font size="2">第
							${page.pageNow} 页</font> <a href="rest/qmp/factoryTotal?pageNow=1"
							class='search_A'>首页</a>
						<c:choose>
							<c:when test="${page.pageNow - 1 > 0}">
								<a href="rest/qmp/factoryTotal?pageNow=${page.pageNow - 1}"
									class='search_A'>上一页</a>
							</c:when>
							<c:when test="${page.pageNow - 1 <= 0}">
								<!-- <a href="rest/qmp/factoryTotal?pageNow=1" class='search_A'>上一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${page.totalPageCountZcfg==0}">
								<a href="rest/qmp/factoryTotal?pageNow=${page.pageNow}"
							...
							<c:when test="${page.pageNow + 1 > page.totalPageCountZcfg}">
								<!-- <a href="rest/qmp/factoryTotal?pageNow=${page.totalPageCount}"
									class='search_A'>下一页</a> -->
							</c:when>
						</c:choose>
						<c:choose>
							...
							<c:otherwise>
								<a href="rest/qmp/factoryTotal?pageNow=${page.totalPageCountZcfg}"
									class='search_A'>尾页</a>
							</c:otherwise>
						</c:choose>
	...
	<script type="text/javascript" src="app/js/qmp/kzdmFactory.js"></script>
</body>
</html>